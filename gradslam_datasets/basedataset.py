...
class GradSLAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.name = config_dict["dataset"]
        self.device = device
        self.png_depth_scale = config_dict["cam"]["png_depth_scale"]

        self.orig_height = config_dict["cam"]["image_height"]
        self.orig_width = config_dict["cam"]["image_width"]
        self.fx = config_dict["cam"]["fx"]
        self.fy = config_dict["cam"]["fy"]
        self.cx = config_dict["cam"]["cx"]
        self.cy = config_dict["cam"]["cy"]
        ...
        self.distortion = (
            np.array(config_dict["cam"]["distortion"])
            if "distortion" in config_dict["cam"]
            else None
        )
        self.crop_size = (
            config_dict["cam"]["crop_size"]
            if "crop_size" in config_dict["cam"]
            else None
        )

        self.crop_edge = None
        if "crop_edge" in config_dict["cam"].keys():
            self.crop_edge = config_dict["cam"]["crop_edge"]
        ...
        if hasattr(self, "load_imu"):
            self.tstamps = self.load_tstamps()
            self.tstamps = self.tstamps[self.start : self.end : stride]
            self.imus = self.load_imu()
            # TODO: For stride, we need to combine
            concat_imus = []
            idx = 0
            while idx < self.end:
    ...
    def __getitem__(self, index):
        ...
        imu = None  # Dummy variable
        if hasattr(self, "load_imu"):
            imu = self.imus[index]
            imu = imu.to(self.device).type(self.dtype)
