...
def evaluate_image_quality(img1, img2, img2_depth):
    # mask = img2_depth > 0
    # wimg1 = img1 * mask
    # wimg2 = img2 * mask
    psnr_score = psnr(img1, img2).mean()
...
def evaluate_ate_rmse(est_poses, gt_poses, method="umeyama"):
    """
    Aligns est_poses with gt_poses and returns the ATE RMSE
    """
    assert len(est_poses) == len(
        gt_poses
    ), "Estimated trajectory and GT trajectory must have equal length"

    if isinstance(est_poses, torch.Tensor):
        est_traj = est_poses[:, 4:].detach().cpu().numpy()
        est_poses_aligned = est_poses.clone().detach().cpu().numpy()
    else:
        est_traj = est_poses[:, 4:]
        est_poses_aligned = np.copy(est_poses)
    if isinstance(gt_poses, torch.Tensor):
        gt_traj = gt_poses[:, 4:].detach().cpu().numpy()
    else:
        gt_traj = gt_poses[:, 4:]

    if method.lower() == "horn":
        est_traj = est_traj.T
        gt_traj = gt_traj.T
        rot, trans, ate = align(est_traj, gt_traj)

        est_poses_aligned[:, :4] = (
            rotation2quad(
                torch.matmul(
                    torch.tensor(rot).cuda().float(),
                    quad2rotation(est_poses[:, :4]).float(),
                )
            )
            .cpu()
            .numpy()
        )
        est_poses_aligned[:, 4:] = (rot * est_traj + trans).T

    elif method.lower() == "umeyama":
        s, rot, trans = align_umeyama(gt_traj, est_traj)
        rot = np.asmatrix(rot)
        ...
        est_poses_aligned[:, 4:] = (s * (rot * est_traj.T) + trans).T

        ate = np.linalg.norm(est_poses_aligned[:, 4:] - gt_traj, axis=1)

    else:
        ate = np.linalg.norm(est_traj - gt_traj, axis=1)

    ate_rmse = np.sqrt(np.dot(ate, ate) / len(ate))

    if isinstance(est_poses, torch.Tensor):
        est_poses_aligned = torch.tensor(est_poses_aligned)

    return est_poses_aligned, ate_rmse
